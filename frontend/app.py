...
def main():
    ...
    # Pages
    pages = [
        'Company Information', 'Data Collection and Usage',
        'Data Subject Rights', 'Data Sharing and Transfers',
        'Data Retention and Security', 'Cookies Usage',
        'Laws and Dispute Resolution', 'Summary'
    ]
            ...
            if st.button('Confirm'):
                with open('output.md', 'w') as f:
                    for key, value in SECTION_NAMES.items():
                        f.write(f"## {key}: {value}\n")
                        f.write(
                            f"{st.session_state['from_back'].get(key, 'No data')}\n\n"
                        )
                st.success('Data saved to output.md!')
                # if validate_page(st.session_state['page']):
                #     save_current_page_data(pages[st.session_state['page']])
...
def suggestions_input():
    ...
    with col2:
        user_input = st.text_input("Enter info:",
                                   st.session_state.get('user_input', ''))

    with col3:
        if st.button("Submit"):
            st.success(f"Data for {selected_category} received: {user_input}")
            if st.session_state['from_back'][
                    selected_category] != {} and user_input != 'regenerate':
                ...
                syl = json.dumps(
                    {
                        SECTION_NAMES[selected_category]:
                        st.session_state['from_back']['syl'][
                            SECTION_NAMES[selected_category]]
                    },
                    indent=4)
                try:
                    ...
                    response = requests.post(
                        ...
                        data={
                            "section_name": SECTION_NAMES[selected_category],
                            "section_text": text,
                            "syllabus": syl,
                            "regulations": reg,
                    ...
                    if response.status_code == 200:
                        ...
                        st.session_state['from_back'][
                            f'{selected_category}'] = section['content']
                        print(st.session_state['from_back']
                              [f'{selected_category}'])
                        if DISPLAY_TEMPLATE:
                ...
                st.session_state['from_back'][
                    f'{selected_category}_flag'] = True
            else:
...
def save_current_page_data(page):
    ...
    if page == 'Company Information':
        ...
        if form_data['area'] != st.session_state.get('area', ''):
            ...
            try:
                # Send a request to obtain the regulations of the corresponding region
                response = requests.post(
                    f"{API_URL}/regulations",
                    data={"areas": st.session_state.get('area', '')})
                if response.status_code == 200:
                    regulations = response.json()
                    st.session_state['from_back']['reg'] = [
                        reg["regulations"] for reg in regulations
                    ]
                    if DISPLAY_TEMPLATE:
                    ...
                    try:
                        ...
                        response = requests.post(
                            ...
                            data={
                                "regulations":
                                ",".join(st.session_state['from_back']['reg'])
                            })
                        if response.status_code == 200:
                            st.session_state['from_back'][
                                'syl'] = response.json()
        ...
        if (form_data['company_name'] != st.session_state.get(
                'company_name', '') or form_data['address']
        ...
        if (form_data['data_types'] != st.session_state.get('data_types', [])
                or form_data['collection_methods'] != st.session_state.get(
                    'collection_methods', [])
                or form_data['usage_purposes'] != st.session_state.get(
                    'usage_purposes', [])
                or form_data['legal_basis'] != st.session_state.get(
                    'legal_basis', [])):
            renew_flag(3)

        form_data['data_types'] = st.session_state.get('data_types', [])
        form_data['collection_methods'] = st.session_state.get(
            'collection_methods', [])
        form_data['usage_purposes'] = st.session_state.get(
            'usage_purposes', [])
        ...
        if (form_data['user_rights'] != st.session_state.get(
                'user_rights', [])
                or form_data['collect_child_data'] != st.session_state.get(
                    'collect_child_data', False)
                or form_data['child_protection_measures']
                != st.session_state.get('child_protection_measures', [])):
            renew_flag(4)

        form_data['user_rights'] = st.session_state.get('user_rights', [])
        form_data['collect_child_data'] = st.session_state.get(
            'collect_child_data', False)
        form_data['child_protection_measures'] = st.session_state.get(
            'child_protection_measures', [])

        st.session_state['from_back']['flag'][3] = True
    elif page == 'Data Sharing and Transfers':
        # Save data sharing and transfers info
        if (form_data['sharing_partners'] != st.session_state.get(
                'sharing_partners', [])
                or form_data['international_transfer'] != st.session_state.get(
                    'international_transfer', False)
                or form_data['transfer_measures'] != st.session_state.get(
                    'transfer_measures', [])):
            renew_flag(5)

        form_data['sharing_partners'] = st.session_state.get(
            'sharing_partners', [])
        form_data['international_transfer'] = st.session_state.get(
            'international_transfer', False)
        form_data['transfer_measures'] = st.session_state.get(
            'transfer_measures', [])

        st.session_state['from_back']['flag'][4] = True
    elif page == 'Data Retention and Security':
        # Save data retention and security info
        if (form_data['retention_period']
                != st.session_state.get('retention_period', '')):
            renew_flag(6)

        if (form_data['protection_measures']
                != st.session_state.get('protection_measures', [])):
            renew_flag(8)

        form_data['retention_period'] = st.session_state.get(
            'retention_period', '')
        form_data['protection_measures'] = st.session_state.get(
            'protection_measures', [])
        ...
        if (form_data['use_cookies']
                != st.session_state.get('use_cookies', False)):
            renew_flag(10)
        ...
        if (form_data['dispute_resolution']
                != st.session_state.get('dispute_resolution', '')):
            renew_flag(12)

        if (st.session_state['uploaded_files']
                != st.session_state.get('uploaded_files', None)):
            renew_flag(0)
            try:
                # Send a request to generate a course outline for the corresponding regulations
                response = requests.post(
        ...
        form_data['dispute_resolution'] = st.session_state.get(
            'dispute_resolution', '')
        st.session_state['uploaded_files'] = [
            file.name for file in st.session_state.get('uploaded_files', [])
            if file is not None
        ]
    ...
    if st.session_state['from_back']['syl'] != {}:
        ...
        for key, value in SECTION_NAMES.items():
            #syl = json.dumps(st.session_state['from_back']['syl'][f'{value}'], indent=4)
            syl = json.dumps(
                {value: st.session_state['from_back']['syl'][value]}, indent=4)
            if st.session_state['from_back'][f'{key}_flag'] == False:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        section = response.json()
                        st.session_state['from_back'][f'{key}'] = section[
                            'content']
...
def display_page(page):
    ...
    if page == 'Company Information':
        st.session_state['company_name'] = st.text_input(
            'Company Name',
            st.session_state['form_data'].get('company_name', ''))
        st.session_state['address'] = st.text_input(
            'Address', st.session_state['form_data'].get('address', ''))
        st.session_state['area'] = st.text_input(
            'Service Area', st.session_state['form_data'].get('area', ''))
        ...
        st.session_state['data_types'] = st.multiselect(
            'Types of Data Collected', DATA_TYPES,
            st.session_state['form_data'].get('data_types', []))
        st.session_state['collection_methods'] = st.multiselect(
            'Data Collection Methods',
            ['Directly from you', 'Automatically', 'From third parties'],
            st.session_state['form_data'].get('collection_methods', []))
        st.session_state['usage_purposes'] = st.multiselect(
            'Data Usage Purposes', USAGE_PURPOSES,
            st.session_state['form_data'].get('usage_purposes', []))
        st.session_state['legal_basis'] = st.multiselect(
            'Legal Basis for Processing', LEGAL_BASIS,
            st.session_state['form_data'].get('legal_basis', []))

    elif page == 'Data Subject Rights':
        st.session_state['user_rights'] = st.multiselect(
            'User Rights', USER_RIGHTS,
            st.session_state['form_data'].get('user_rights', []))
        st.session_state['collect_child_data'] = st.checkbox(
            'Collect Data from Children?',
            st.session_state['form_data'].get('collect_child_data', False))
        if st.session_state['collect_child_data']:
            st.session_state['child_protection_measures'] = st.multiselect(
                'Child Data Protection Measures',
                ['Parental Consent', 'Child Data Encryption'],
                st.session_state['form_data'].get('child_protection_measures',
                                                  []))

    elif page == 'Data Sharing and Transfers':
        st.session_state['sharing_partners'] = st.multiselect(
            'Data Sharing Partners', SHARING_PARTNERS,
            st.session_state['form_data'].get('sharing_partners', []))
        st.session_state['international_transfer'] = st.checkbox(
            'Transfer Data Internationally?',
            st.session_state['form_data'].get('international_transfer', False))
        if st.session_state['international_transfer']:
            st.session_state['transfer_measures'] = st.multiselect(
                'International Transfer Measures', TRANSFER_MEASURES,
                st.session_state['form_data'].get('transfer_measures', []))

    elif page == 'Data Retention and Security':
        st.session_state['retention_period'] = st.text_input(
            'Data Retention Period',
            st.session_state['form_data'].get('retention_period', ''))
        st.session_state['protection_measures'] = st.multiselect(
            'Data Protection Measures', PROTECTION_MEASURES,
            st.session_state['form_data'].get('protection_measures', []))

    elif page == 'Cookies Usage':
        st.session_state['use_cookies'] = st.checkbox(
            'Use Cookies and Tracking Technologies?',
            st.session_state['form_data'].get('use_cookies', False))

    elif page == 'Laws and Dispute Resolution':
        st.session_state['dispute_resolution'] = st.text_input(
            'Dispute Resolution Mechanism',
            st.session_state['form_data'].get('dispute_resolution', ''))
